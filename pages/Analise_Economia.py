...
if data_inicial and data_final:
    ...
    df_resumo_performance_tipo_veiculo = df_filtro_data.groupby('Tipo de Veículo').agg({'Km Rodado': 'sum', 'Litros': 'sum', 'Litros Consumidos Meta': 'sum', 'Valor Total': 'sum'}).reset_index()

    df_resumo_performance_tipo_veiculo_geral_colunas = gerar_df_grid(df_resumo_performance_tipo_veiculo, {'Litros': 'Litros Consumidos Real'}, 
    ...
    if selected_rows is not None and len(selected_rows)>0:
        ...
        if tipo_veiculo:
            ...
            df_resumo_performance_veiculo = df_filtro_data[df_filtro_data['Tipo de Veículo']==tipo_veiculo].groupby('Veiculo')\
                .agg({'Km Rodado': 'sum', 'Litros': 'sum', 'Litros Consumidos Meta': 'sum', 'Valor Total': 'sum'}).reset_index()
            
            df_resumo_performance_veiculo_geral_colunas = gerar_df_grid(df_resumo_performance_veiculo, {'Litros': 'Litros Consumidos Real', 'Veiculo': 'Veículo'}, 
                                                                        ['Veículo', 'Km Rodado', 'Litros Consumidos Real', 'Litros Consumidos Meta', 'Economia'])
            ...
            if selected_rows_2 is not None and len(selected_rows_2)>0:
                ...
                df_resumo_performance_motorista_veiculo_geral_colunas = gerar_df_grid(df_resumo_performance_motorista_veiculo, {'Litros': 'Litros Consumidos Real'}, 
